...
class QuizViewModel {
    ...
    fun saveIncorrectWord(quizWord: QuizWord) {
        if (quizWord !in incorrectWords){
            ...
            RankingRepository.profile.incorrectQuiz = RankingRepository.profile.incorrectQuiz.plus(incorrectWords.toString())
        }
    }
    ...
    fun saveCalculateCorrectRate(correctRate: Float){
        RankingRepository.profile.correctRate = correctRate
    }
}
